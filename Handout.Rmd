...
You have created the new variable `bmi`. You can also use the `mutate()` function to change the vector type of a variable. 
...
Let's make a variable with units recorded along with the values. Then try to calculate the mean weight.
...
# The variable is now numeric, and we can calculate the weight.
mean(weight)
```
...
What if we decide we don't want non central temperatures?
...
```{r, fig.width=5, fig.height =2}
ggplot(data = cchic) +
  geom_bar(mapping = aes(x = medical, fill = sex), position = "dodge")
```
...
The best way to describe normally distributed data is by reporting its mean and standard deviation. The mean (average) is the sum of all values in a variable divided by the number of values. The standard deviation is a description of the spread of values in a variable (distributions may have similar means but very different spreads). 
...
Do you notice the wider spread of data in the plot of data with the higher standard deviation?
...
We need to change `los` to a numeric vector, to let us run tests using it. 
...
Read the help page and ensure that your input vectors meet the limits of the arguments for the function `wilcox.test()`.
